...
;_acumular_en_tabla_fin: Copia los ultimos 8 numeros que se presionaron (y que estan en la tabla aux) en la tabla final en 0x0021000 esp
...
isr_tick:
	pushad
    inc  dword[__VARIABLES]
    cmp  dword[__VARIABLES], 50; la interrupcion se activa cada 100 ms -> x lo tanto, tengo que comparar este flag con 50
    JGE  pantalla

    jmp fin_int

pantalla:
    mov dword[__VARIABLES], 0 ; reset de la variable del timer
    push ebp
    mov ebp, esp
